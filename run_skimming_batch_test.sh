...
find "$INPUT_DIR" -name "*.dk2nu.root" | head -n 12 > filelist_12.txt

echo "Starting test skimming with $NJOBS parallel jobs on 12 files..."
cat filelist_12.txt | xargs -n 1 -P $NJOBS -I{} bash -c '
  infile="{}"
...
echo "est batch completed on 12 files."
echo "DK2NU skims to dk2nu_skims/"
echo "Alcove skims to alcove_skims/"
echo "Logs to logs/"
